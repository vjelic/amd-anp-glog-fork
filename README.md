...
6.  **Build With Telemetry Enabled:**
    ...
    To build the plugin with telemetry features enabled (the `ANP_DEBUG_ENABLED` macro defined), use the build command with flag ANP_DEBUG=1.

    ```bash
    make ANP_DEBUG=1 RCCL_BUILD=$RCCL_BUILD MPI_INCLUDE=$MPI_INCLUDE MPI_LIB_PATH=$MPI_LIB_PATH debug
    ```
    If successful, you will see `librccl-net.so` in the `build/` folder of this plugin project.

    **Example:**
    ```bash
    make ANP_DEBUG=1 RCCL_BUILD=/home/user/rccl/build/release MPI_INCLUDE=/home/user/ompi-4.1.6/install/include/ MPI_LIB_PATH=/home/user/ompi-4.1.6/build/ompi/.libs/
    ```
---
...
To enable telemetry, the plugin reads its configuration from a JSON file, whose location is specified by an environment variable AMD_ANP_CONFIG_FILE.
...
```
- `debug_enabled`: Set to `true` to activate telemetry. This ensures that device status and queue pair statistics are logged.
- `log_level`: Specifies the log level for telemetry logs.
- `log_interval`: Frequency (in seconds) at which data is logged.
...
**Note:** Telemetry can be enabled either by setting debug_enabled in the configuration JSON or through an environment variable AMD_ANP_DEBUG_EN.

```
export AMD_ANP_DEBUG_EN=1
```
